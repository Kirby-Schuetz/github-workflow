...
// create new party
const createNewParty = async (title, event, city, state, country) => {
  try{
    const response = await fetch(PARTIES_API_URL, {
      method: 'POST',
      body: JSON.stringify({title, event, city, state, country}),
      headers: {
...
const renderSinglePartyById = async (id) => {
  try {
    ...
    const giftsResponse = await fetch(`${PARTIES_API_URL}/party/${id}`);
    ...
    console.error(error);
      ...
      deleteButton.addEventListener('click', async (event) => {
        try{
          ...
          const remainingParties = await getAllParties()
          renderParties(remainingParties)
        } catch (error) {
          console.error(error)
        }
